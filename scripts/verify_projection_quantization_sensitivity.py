...
from projection_quantization_sensitivity import ROOT, file_hash, read_and_validate_rows, validate_controls
...
def main():
    ...
    check(json.loads((out / "experiment_config.json").read_text()) == run["config"], "Saved config mismatch")
    ...
    with np.load(out / "sampled_token_blocks.npz", allow_pickle=False) as saved:
    ...
    check(np.array_equal(draws, np.load(out / "paired_bootstrap_indices.npy", allow_pickle=False)), "Bootstrap draw mismatch")
    ...
    if args.compare_smoke is not None:
        smoke = json.loads((args.compare_smoke / "run_manifest.json").read_text())
    ...
    evidence["status"] = "PASS: artifact audit; manual requirement-to-evidence review is still required"
    (out / "artifact_verification.json").write_text(json.dumps(evidence, indent=2, ensure_ascii=False) + "\n")
