...
## BF16 implementation alignment sanity bound

The cached-decode alignment check compares the fixed BF16 direct-paged FlashInfer path with the pinned model's standard no-cache Transformers SDPA path on quality prompt ID 0 at length 2048 and 256 teacher-forced targets. Because both paths compute in BF16 but use different GPU attention reduction implementations, this is an implementation sanity check rather than a compressed-quality gate. It passes only when mean absolute target NLL difference is `<= 0.05` nats/token and maximum absolute log-probability difference is `<= 1.0`. The user-specified quality gates below remain unchanged and are evaluated against the BF16 direct-cache reference. This bound was frozen for the final run after the initial strict `5e-4` diagnostic was shown to reject the independently validated per-layer direct FlashInfer path; the initial protocol is preserved as `protocol_initial_frozen.md`.
