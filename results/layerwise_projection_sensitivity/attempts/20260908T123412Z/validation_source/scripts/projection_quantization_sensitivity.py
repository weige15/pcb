#!/usr/bin/env python3
"""Qwen3 attention-projection weight quantization sensitivity (grouped RTN)."""
import argparse
...
def validate_config(cfg):
    ...
    for key, expected in fixed.items():
        require(cfg[key] == expected, f"Projection quantization sensitivity fixed setting {key}: expected {expected!r}")
...
def prepare_data(cfg, out, manifest):
    ...
    manifest["data"] = {
        ...
        "sample_indices": indices.tolist(), "sample_token_ids": "sampled_token_blocks.npz",
...
def formal(model, indices, blocks, cfg, original_hashes, manifest, out):
    reference = {}
    baseline_nll = {}
    with open(out / "block_metrics.csv", "x", newline="") as f:
...
def read_and_validate_rows(out, manifest):
    with np.load(out / "sampled_token_blocks.npz", allow_pickle=False) as data:
...
def analyze(out, manifest):
    ...
    save_json(out / "sensitivity_analysis.json", {"summary": summary, "paired_comparisons": paired,
                                            "bootstrap": {"seed": 42, "replicates": 2000, "unit": "selected block", "ci": "percentile 95%, linear quantiles"}})
    lines = ["# Qwen3-4B attention projection quantization sensitivity", "",
    ...
    lines += [f"- {len(paired)} 個探索性 paired CI 中 {crossing} 個跨零；跨零不是已證明等價。接受沒有明顯差異，不挑選或加跑條件追求排序。", "",
              "## 控制與可重跑證據", "",
              "- 正式跑前以一個已選 block 的兩個 contexts 跑 BF16/WQ4；控制結果保存在 `run_manifest.json:smoke`。不把控制或 deterministic 重跑當獨立樣本。",
              ...
              "- 詳細 module 名稱、shape、參數量、相對 L2 誤差在 `run_manifest.json:interventions`；Q 每條件 377,487,360 個參數，K/V 各 94,371,840。",
              "- 固定設定與實際命令：`run_manifest.json`；原始逐 block：`block_metrics.csv`；抽樣索引/token IDs：`sampled_token_blocks.npz`；tokenizer 設定：`tokenizer/`。",
              f"- 統計可獨立重算：`python scripts/projection_quantization_sensitivity.py --analyze-only --output {shlex.quote(str(out))}`；bootstrap draws：`paired_bootstrap_indices.npy`；未四捨五入摘要/CI：`sensitivity_analysis.json`。",
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "scripts/projection_quantization_sensitivity_config.json")
    parser.add_argument("--output", type=Path, default=ROOT / "results/projection_quantization_sensitivity")
    ...
    require(not (out / "run_manifest.json").exists() and not (out / "block_metrics.csv").exists(),
            "Output already contains a run. Choose a new --output; do not overwrite evidence.")
    cfg = json.loads(args.config.read_text())
    validate_config(cfg)
    command = shlex.join([sys.executable, *sys.argv])
    save_json(out / "experiment_config.json", cfg)
    rerun = (f"CUDA_VISIBLE_DEVICES={shlex.quote(os.environ.get('CUDA_VISIBLE_DEVICES', '0'))} "
             "bash scripts/run_projection_quantization_sensitivity.sh results/projection_quantization_sensitivity/rerun_$(date -u +%Y%m%dT%H%M%SZ) "
             f"--config {shlex.quote(str(out / 'experiment_config.json'))}" + (" --smoke-only" if args.smoke_only else ""))
    manifest = {"status": "starting", "started_utc": now(), "config": cfg, "command": command,
                ...
                "source_sha256": {os.path.relpath(p, ROOT): file_hash(p) for p in
                                  [ROOT / "scripts/projection_quantization_sensitivity.py", ROOT / "scripts/run_projection_quantization_sensitivity.sh", args.config.resolve(), ROOT / "projection_quantization_sensitivity.md"]},
    ...
    try:
        ...
        artifacts = [out / name for name in ("experiment_config.json", "sampled_token_blocks.npz", "block_metrics.csv",
                                             "paired_bootstrap_indices.npy", "sensitivity_analysis.json", "sensitivity_report.md")]
        ...
        with open(path, "a") as f:
            f.write(f"\n\n## {now()} — 本輪阻塞，未完成\n\nCommand: `{command}`\n\n"
                    f"Completed conditions: {len(manifest['completed_conditions'])}/14. See run_manifest.json for exact completed cells.\n\n"
